/* ═══════════════════════════════════════════════════════════════
   THE BOARDROOM — UI State Management
   Handles screen transitions, dialogue rendering, persona badges,
   phase indicators, decision record reveal, and all user-facing state.
   ═══════════════════════════════════════════════════════════════ */
...
COMPANION.UI = (function () {
  ...
  function cacheElements() {
    ...
    elements = {
      ...
      packetContext: document.getElementById('packet-context'),
      packetConstraints: document.getElementById('packet-constraints'),
      packetOptions: document.getElementById('packet-options'),
  ...
  function updatePhase(phase) {
    ...
    var labels = {
      1: 'Phase I — The Board Packet',
      2: 'Phase II — Convening',
      3: 'Phase III — Truth Round',
      4: 'Phase IV — Proposals',
      5: 'Phase V — Cross-Examination',
      6: 'Phase VI — Motions',
      7: 'Phase VII — Votes',
      8: 'Phase VIII — The Seal'
    };
  ...
  function addPersonaMessage(personaName, color) {
    ...
    header.textContent = personaName || 'The Boardroom';
  ...
  function addDecisionCard(decisionData) {
    if (!elements.dialogueMessages) return;
    ...
    html += '<div class="threshold-chat-heading">The Seal &mdash; Decision Record</div>';
    html += '<div class="threshold-chat-divider-line"></div>';

    html += '<div class="threshold-chat-job">';
    html += '<div class="threshold-chat-job-title">' + escapeHtml(decisionData.decision || 'Decision Rendered') + '</div>';
    if (decisionData.rationale) {
      html += '<div class="threshold-chat-job-company">' + escapeHtml(decisionData.rationale) + '</div>';
    }
    if (decisionData.refused) {
      html += '<div class="threshold-chat-job-location">Refused: ' + escapeHtml(decisionData.refused) + '</div>';
    }
    if (decisionData.proof) {
      html += '<div class="threshold-chat-job-salary">Prove in 30 days: ' + escapeHtml(decisionData.proof) + '</div>';
    }
    html += '</div>';
  ...
  function updateHint(phase, activeCount) {
    ...
    if (phase <= 2) {
      elements.inputHint.innerHTML =
        '<span class="hint-text">The Chair is convening. State your case.</span>';
    } else if (phase <= 5) {
      elements.inputHint.innerHTML =
        '<span class="hint-text">The Board deliberates. Interject if needed.</span>';
    } else if (phase <= 7) {
      elements.inputHint.innerHTML =
        '<span class="hint-text">Motions and votes. The Board is converging.</span>';
    } else if (phase === 8) {
      elements.inputHint.innerHTML =
        '<span class="hint-text">The Seal has been affixed. The decision record is above.</span>';
    } else if (activeCount === 0) {
      elements.inputHint.innerHTML =
        '<span class="hint-text">The Boardroom awaits.</span>';
    }
  }
  ...
  function showLoading(msg) {
    var existing = document.getElementById('boardroom-loading');
    if (existing) existing.remove();
    if (!elements.dialogueMessages) return;
    var el = document.createElement('div');
    el.id = 'boardroom-loading';
  ...
  function hideLoading() {
    var el = document.getElementById('boardroom-loading');
    if (el) el.remove();
  }
  ...
  function init() {
    cacheElements();
    if (elements.userInput) {
      elements.userInput.placeholder = 'Address the Board...';
  ...
  return {
    ...
    addDecisionCard: addDecisionCard,
