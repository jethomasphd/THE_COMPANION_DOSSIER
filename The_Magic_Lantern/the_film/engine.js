...
/* ── cancellable + pausable + scrubbable timing ── */
let token = null;
const SPEED = 1.1; /* global pace · <1 is faster · the whole film scales by this · slowed a beat to read */
let INSTANT = false;          /* when true, waits resolve at once — used to rebuild state when seeking */
let WPAUSED = false;          /* when true, every in-flight wait is frozen until resumed */
const activeWaits = new Set();
function newToken(){ if(token) cancelTok(token); token={cancelled:false,timers:[],rej:[]}; return token; }
function wait(ms){
  const tk=token; ms=Math.max(0,(ms||0)*SPEED); if(INSTANT) ms=0;
  return new Promise((res,rej)=>{
    let remaining=ms, t=null, startAt=0, done=false;
    function fin(){ if(done) return; done=true; activeWaits.delete(w); res(); }
    function arm(){ startAt=performance.now(); t=setTimeout(fin, Math.max(0,remaining)); }
    const w={
      pause(){ if(t){ clearTimeout(t); t=null; remaining-=performance.now()-startAt; } },
      resume(){ if(!t && !done && !tk.cancelled) arm(); },
      cancel(){ if(done) return; done=true; if(t) clearTimeout(t); activeWaits.delete(w); rej('x'); }
    };
    activeWaits.add(w); tk.rej.push(()=>w.cancel());
    if(!WPAUSED) arm();
  });
}
function cancelTok(tk){ tk.cancelled=true; tk.timers.forEach(clearTimeout); tk.rej.forEach(r=>r('x')); }
function pauseWaits(){ if(WPAUSED) return; WPAUSED=true; activeWaits.forEach(w=>w.pause()); }
function resumeWaits(){ if(!WPAUSED) return; WPAUSED=false; activeWaits.forEach(w=>w.resume()); }
...
function threshold(){ if(!actx||!soundOn||INSTANT) return; const n=actx.currentTime;
...
function glass(ms){ if(!actx||!soundOn||INSTANT) return; const n=actx.currentTime, dur=Math.min((ms||3000)/1000,3.4);
...
function tick(){ if(!actx||!soundOn||INSTANT) return; const n=actx.currentTime;
...
function chime(){ if(!actx||!soundOn||INSTANT) return; const n=actx.currentTime;
...
function knell(){ if(!actx||!soundOn||INSTANT) return; const n=actx.currentTime;
...
/* ── text helpers ── */
async function type(el,text,per){ if(INSTANT){ el.textContent=text; return; } el.textContent=''; for(let i=0;i<text.length;i++){ el.textContent=text.slice(0,i+1); if(text[i]!==' ') tick(); await wait(per); } }
...
async function termType(d,text,per){ if(INSTANT){ d.textContent=text; return; } d.textContent=''; for(let i=0;i<text.length;i++){ d.textContent=text.slice(0,i+1); if(text[i]!==' ') tick(); await wait(per); } }
...
async function exec(s){
  switch(s.t){
    ...
    case 'sub':
      ...
      s.scrim ? $('subs').classList.add('onchart') : $('subs').classList.remove('onchart');
