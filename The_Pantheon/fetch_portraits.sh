...
# slug | output filename | explicit url (empty => resolve via Wikipedia summary API) | fallback slug (optional)
#
# Fallback slug is tried if the primary slug returns a disambiguation page or
# no usable thumbnail. Use it when the bare name resolves to a disambiguation
# (e.g. John_Snow → John_Snow_(physician)).
PORTRAITS=(
  ...
  "John_Snow|John_Snow.jpg||John_Snow_(physician)"
  "Michael_Marmot|Michael_Marmot.jpg||"
  "Carl_Jung|Carl_Jung.jpg||"
  "Paul_Farmer|Paul_Farmer.jpg||Paul_Farmer_(physician)"
...
try:
    ...
    sys.exit(1)
# Disambiguation pages have no real portrait — reject so caller can try a fallback.
...
for entry in "${PORTRAITS[@]}"; do
  IFS='|' read -r slug filename explicit_url fallback_slug <<< "$entry"
  ...
  if [ -z "$img_url" ]; then
    ...
    if [ -z "$img_url" ] && [ -n "$fallback_slug" ]; then
      printf "  … %-32s primary slug yielded no portrait — trying fallback %s\n" "$slug" "$fallback_slug"
      img_url="$(resolve_via_api "$fallback_slug" 2>/dev/null || true)"
    fi
  fi
...
# A few stubborn slugs shouldn't block the workflow from committing the
# portraits we did manage to fetch. Only fail hard when nothing succeeded.
if [ "$ok" -eq 0 ] && [ "$skipped" -eq 0 ] && [ "$failed" -gt 0 ]; then
  exit 1
fi
exit 0
