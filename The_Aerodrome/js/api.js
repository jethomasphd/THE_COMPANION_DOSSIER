/* ═══════════════════════════════════════════════════════════════
   THE AERODROME — Claude API Integration
   Handles streaming communication with Anthropic's Claude API,
   directly or through the COMPANION Cloudflare Worker proxy.

   Supports pre-configured proxy/key via config.js with client-side
   safeguards: rate limiting, session caps, daily budget tracking.
   ═══════════════════════════════════════════════════════════════ */
...
COMPANION.API = (function () {
  ...
  const STORAGE_KEY_MSG_COUNT = 'the_aerodrome_v1_msg_count';
  const STORAGE_KEY_SESSION_START = 'the_aerodrome_v1_session_start';
  ...
  function checkCooldown() {
    ...
    if (lastRequestTime > 0 && elapsed < guards.cooldownSeconds) {
      return { allowed: false, message: 'Let the air settle a moment before you speak again.' };
    }
    return { allowed: true };
  }
  ...
  function checkSessionMessageLimit() {
    ...
    if (count >= guards.maxMessagesPerSession) {
      return { allowed: false, message: 'This flight has reached its length (' + guards.maxMessagesPerSession + ' exchanges). Refresh to take off again.' };
    }
    return { allowed: true };
  }
  ...
  function checkDailySessionLimit() {
    ...
    if (count >= guards.maxSessionsPerDay) {
      return { allowed: false, message: 'Daily flight limit reached. Return tomorrow — the wind will keep.' };
    }
    return { allowed: true };
  }
  ...
  function checkSessionTimeout() {
    ...
    if (start > 0) {
      ...
      if (elapsed > guards.sessionTimeoutMinutes) {
        return { allowed: false, message: 'This flight has ended. Refresh to climb again.' };
      }
    }
    return { allowed: true };
  }
  ...
  // ── Session Persistence ──
  const SESSION_STORAGE_KEY = 'companion_aerodrome_session';
