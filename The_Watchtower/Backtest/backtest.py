...
Reproducibility note (added for the repo)
-----------------------------------------
The original notebook used END = today (a *rolling* 10-year window). Here END is
pinned to 2025-12-31 so the backtest ends exactly where the live Watchtower vigil
begins (2026-01-02) and the two never overlap. Set END = pd.Timestamp.today() to
get the live rolling window instead. Everything else is unchanged. The committed
CSV artifacts in this folder are the output of this script for the pinned window.
"""
...
# matplotlib is optional (only used for the on-screen plot); guard it so the
# script still runs and exports CSVs in headless / server environments.
try:
...
# Pinned so the backtest does not overlap the live 2026 vigil.
# Use END = pd.Timestamp.today().normalize() for a live rolling 10-year window.
...
def _download(tickers):
    """Download via yfinance. In restricted/proxied networks yfinance's default
    curl_cffi transport can be reset; passing a plain requests Session (which
    honors HTTPS_PROXY / REQUESTS_CA_BUNDLE) is a reliable fallback. On a normal
    network the plain call works and this try/except is a no-op."""
    kw = dict(start=START.strftime("%Y-%m-%d"),
    ...
    try:
        ...
        return yf.download(tickers, session=sess, **kw)
    except TypeError:
...
if _HAVE_MPL:
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig("republic_vs_spy_equity_10y.png", dpi=150)
    print("\nSaved plot: republic_vs_spy_equity_10y.png")
