...
*Files are added by the authors; until each appears, the row above is its placeholder.*
...
The Prism Experiment depends on stimulus materials that must demonstrably exist **before** the experiment is run. To make that verifiable rather than merely asserted, the vignette document is deposited here **sealed and timestamped**:

- The **timestamp** — the git history together with the file's own seal — fixes the contents in advance, on the record.
...
While the preprint is pre-SSRN, please cite the repository and the Dossier DOI:
