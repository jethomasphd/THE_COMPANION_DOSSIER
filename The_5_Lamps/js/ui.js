/* ═══════════════════════════════════════════════════════════════
   THE FIVE LAMPS — UI State Management
...
COMPANION.UI = (function () {
  ...
  function addPersonaMessage(personaName, color) {
    ...
    header.textContent = personaName || 'The Ward';
  ...
  function updateHint(activePersonaCount) {
    ...
    if (activePersonaCount === 0) {
      elements.inputHint.innerHTML =
        '<span class="hint-text">Say <code>Using this matter, summon Hippocrates.</code> to light a lamp.</span>';
    } else if (activePersonaCount === 1) {
      elements.inputHint.innerHTML =
        '<span class="hint-text">Address the lamp, or <code>Now summon Snow to join.</code></span>';
    } else if (activePersonaCount < 5) {
      elements.inputHint.innerHTML =
        '<span class="hint-text">Address the Ward. <code>Now summon Jung to join.</code> or <code>Release Snow.</code></span>';
    } else {
      elements.inputHint.innerHTML =
        '<span class="hint-text">All five lamps are lit. Present your clinical dilemma.</span>';
    }
  }
  ...
  function renderMarkdownLight(text) {
    ...
    // Bold — also detect speaker headers like **[Dr. Hippocrates]:**
  ...
  function init() {
    ...
    if (elements.userInput) {
      elements.userInput.placeholder = 'Present your dilemma to the Inner Ward...';
