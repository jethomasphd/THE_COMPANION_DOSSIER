/* ═══════════════════════════════════════════════════════════════
   NARRATIVES — Claude API Integration
...
NARRATIVES.API = (function () {

  var API_VERSION = '2023-06-01';
  var MAX_TOKENS = 4096;

  var STORAGE_PREFIX = 'narratives_chat_v1_';
  var STORAGE_KEY_MSG_COUNT = STORAGE_PREFIX + 'msg_count';
  var STORAGE_KEY_SESSION_START = STORAGE_PREFIX + 'session_start';
  var STORAGE_KEY_SESSION_COUNT = STORAGE_PREFIX + 'session_count';
  var STORAGE_KEY_SESSION_DATE = STORAGE_PREFIX + 'session_date';
  ...
  function checkCooldown() {
    ...
    if (lastRequestTime > 0 && elapsed < guards.cooldownSeconds) {
      return { allowed: false, message: 'The room is still settling. Wait a moment.' };
    }
    return { allowed: true };
  }
  ...
  function checkSessionTimeout() {
    ...
    if (start > 0) {
      ...
      if (elapsed > guards.sessionTimeoutMinutes) {
        return { allowed: false, message: 'The session expired. The room went quiet.' };
      }
    }
    return { allowed: true };
  }
  ...
  async function sendMessage(userMessage, systemPrompt, onChunk, onDone, onError) {
    ...
    if (!isProxy) {
      onError('The connection has been lost.');
      return;
    }
